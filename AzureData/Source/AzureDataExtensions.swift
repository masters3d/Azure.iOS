...
public extension DocumentCollection {
    ...
    public func delete(_ trigger: Trigger, callback: @escaping (DataResponse) -> ()) {
        return self.delete(triggerWithId: trigger.id, callback: callback)
    }
    ...
    public func replace (triggerWithId id: String, triggerResourceId: String, operation: Trigger.TriggerOperation, type: Trigger.TriggerType, andBody body: String, callback: @escaping (Response<Trigger>) -> ()) {
        return DocumentClient.default.replace (triggerWithId: id, triggerResourceId: triggerResourceId, operation: operation, type: type, andBody: body, in: self, callback: callback)
    }
}
